...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Print and inspect the Kaggle 20k mBART extraction/training runbook."
    )
    parser.add_argument(
        "--mode",
        choices=["commands", "status", "json"],
        default="commands",
        help="commands prints notebook cells, status inspects local outputs, json emits status JSON.",
    )
...
def format_commands(config: KaggleRunConfig) -> str:
    return "\n\n".join(
        [
            ...
            "# Cell 8: full subset extraction, safe to rerun for resume\n"
            f"{extract_command(config, config.feature_dir, max_examples=None, chunk_size=config.chunk_size)}\n"
...
def collect_status(config: KaggleRunConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "features": {
            ...
            "chunk_reports": len(list((config.feature_dir / "chunks").glob("*.report.json")))
